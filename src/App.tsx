    ...
    <div style={{ background: C.bg, height: "100%", color: C.text, fontFamily: (FONTS as any)[fontFamily], display:"flex", flexDirection:"column", overflow:"hidden", position:"relative", zoom: String(fontScale) }}>
      ...
      <div style={{ padding: isMobile ? "8px 15px" : "11px 18px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0, background: isMobile ? C.bg : "rgba(15,23,42,0.97)", backdropFilter: isMobile ? "none" : "blur(8px)", zIndex:10 }}>
        ...
        <div style={{ display:"flex", gap: isMobile ? 12 : 6, alignItems:"center" }}>
          {isMobile ? (
            <>
              {(primaryTab === "stream" || primaryTab === "lists" || primaryTab === "calendar") && <button onClick={() => { setShowMobileSearch(s => !s); }} title="Search" style={{ background: showMobileSearch ? `${C.accent}22` : "none", border:"none", color: showMobileSearch ? C.accent : C.muted, cursor:"pointer", padding:4, display:"flex", borderRadius:6 }}>
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden", minHeight:0, zIndex:1 }}>
        ...
        {!isMobile && (
            ...
            {!isMobile && (
              <div style={{ width:240, borderRight:`1px solid ${C.border}`, display:"flex", flexDirection:"column", flexShrink:0 }}>
                ...
                <div style={{ flex:1, overflowY:"auto", padding:"0 10px" }}>
                  {[...lists].sort((a,b) => ((b as any).pinned ? 1 : 0) - ((a as any).pinned ? 1 : 0)).map(list => {
                    ...
                    return (
                      ...
                      <button
                        onClick={() => setLists(prev => prev.map(l => l.id === list.id ? { ...l, pinned: !l.pinned } : l))}
                        title={list.pinned ? "Unpin" : "Pin to top"}
                        style={{ background:"none", border:"none", cursor:"pointer", padding:"2px 4px", color: list.pinned ? "#fde047" : C.dimmer, fontSize:13, flexShrink:0, lineHeight:1, marginLeft:-4 }}
                      >★</button>
                      </div>
                    );
                  })}
          ...
          <div style={{ flex:1, overflowY:"auto", padding: isMobile ? "20px 20px 68px" : 40 }}>
