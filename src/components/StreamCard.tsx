        ...
        <motion.div
          ...
          onTouchMove={e => {
            ...
            if (!(e.currentTarget as any)._swipeLocked) {
              if (Math.abs(dy) > Math.abs(dx) && Math.abs(dy) > 10) { (e.currentTarget as any)._swipeStartX = undefined; return; }
          ...
          <div style={{ display:"flex", gap: isMobile ? 16 : 10 }}>
            {isMobile && (
              <div style={{ width:40, height:40, borderRadius:10, background: `${meta.color}15`, border:`1px solid ${meta.color}33`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>
                {entry.emoji || (entry.type === "task" ? "\u26A1" : entry.type === "event" ? "\u{1F4C5}" : entry.type === "note" ? "\u{1F4DD}" : "\u{1F4A1}")}
              </div>
            )}
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom: isMobile ? 10 : 0 }}>
                ...
                {entry.dueDate && (
                  <span style={{ fontSize:11, color: overdueEntry ? "#ef4444" : C.dim, display:"flex", alignItems:"center", gap:4 }}>
                    <span style={{ fontSize:12 }}>{overdueEntry ? "\u26A0\uFE0F" : "\u{1F4C5}"}</span>
              ...
              >
                <div style={{ marginTop:16, paddingTop:16, borderTop:`1px solid ${C.border}` }}>
                  <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:10 }}>
                    <button onClick={onExpand} style={{ fontSize:11, background:"none", border:`1px solid ${C.border}`, color: C.dim, cursor:"pointer", padding:"3px 11px", borderRadius:6, fontFamily:"inherit" }}>&#8593; Close</button>
                  </div>
                  ...
                  {entry.type === "task" && (
                    <div style={{ marginBottom:14 }}>
                      ...
                      {entry.subtasks.length === 0 && <div style={{ fontSize:11, color: C.dimmer, marginBottom:8, fontStyle:"italic" }}>No sub-tasks yet.</div>}
                      ...
                      <div style={{ display:"flex", gap:6, marginTop:6 }}>
                        <input value={stInputs[entry.id] || ""} onChange={e => onSubtaskInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") onSubtaskAdd(); }} placeholder="Add sub-task..." style={{ flex:1, background:C.input, border:`1px solid ${C.border}`, borderRadius:6, padding:"5px 9px", fontSize:12, color: C.text, fontFamily:"inherit", outline:"none" }} />
                  ...
                  <div style={{ marginBottom:14 }}>
                    ...
                    {(entry.comments||[]).map((comment: any) => (
                      <div key={comment.id} style={{ marginBottom:8, padding:"7px 10px", background: C.bg, borderRadius:7, border:`1px solid ${C.border}` }}>
                        {editingComment && editingComment.commentId === comment.id ? (
                          <div>
                            <textarea value={editCommentText} onChange={e => onCommentEditChange(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); onCommentEditSave(); } if (e.key === "Escape") onCommentEditCancel(); }} rows={2} autoFocus style={{ width:"100%", resize:"none", background:C.input, border:`1px solid ${C.accent}55`, borderRadius:5, padding:"5px 8px", fontSize:12, fontFamily:"inherit", color: C.text, outline:"none", boxSizing:"border-box" }} />
                          ...
                          <div style={{ display:"flex", alignItems:"flex-start", gap:7 }}>
                            <div style={{ flex:1 }}>
                              <p onDoubleClick={() => onCommentEditStart(comment.id, comment.text)} title="Double-click to edit" style={{ margin:"0 0 4px", fontSize:12, color: C.text, lineHeight:1.5, wordBreak:"break-word", cursor:"default" }}>{renderMd(comment.text, searchQuery)}</p>
                    ...
                    <div style={{ display:"flex", gap:6 }}>
                      <textarea value={commentInputs[entry.id] || ""} onChange={e => onCommentInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); onCommentAdd(); } }} placeholder="Add a comment... (Enter to submit)" rows={2} style={{ flex:1, resize:"none", background:C.input, border:`1px solid ${C.border}`, borderRadius:7, padding:"6px 9px", fontSize:12, fontFamily:"inherit", color: C.text, outline:"none", lineHeight:1.5, boxSizing:"border-box", transition:"border-color .15s" }} onFocus={e => e.target.style.borderColor=`${C.accent}66`} onBlur={e => e.target.style.borderColor=C.border} />
                      <button onClick={onCommentAdd} disabled={!(commentInputs[entry.id]||"").trim()} style={{ padding:"0 12px", borderRadius:7, border:"none", background:(commentInputs[entry.id]||"").trim() ? C.accent : C.dimmer, color:"#fff", cursor:(commentInputs[entry.id]||"").trim() ? "pointer" : "default", fontSize:13, alignSelf:"stretch", transition:"background .15s" }}>&#8593;</button>
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom:14 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                      ...
                      <input maxLength={4} value={entry.emoji || ""} onChange={e => onEmojiChange(e.target.value.trim())} placeholder="Emoji..." style={{ width:60, background:C.input, border:`1px solid ${C.border}`, borderRadius:6, padding:"4px 9px", fontSize:12, color: C.text, fontFamily:"inherit", outline:"none" }} onClick={e => e.stopPropagation()} />
                      {entry.emoji && <button onClick={e => { e.stopPropagation(); onEmojiChange(""); }} style={{ fontSize:11, background:"none", border:`1px solid ${C.border}`, color: C.dim, cursor:"pointer", padding:"4px 9px", borderRadius:6, fontFamily:"inherit", flexShrink:0 }}>&#10005; Remove</button>}
                    </div>
                    <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                      {["\u2705","\u{1F4DD}","\u{1F680}","\u{1F4A1}","\u{1F4C5}","\u{1F525}","\u2B50","\u{1F4CC}","\u{1F6D2}","\u2708\uFE0F","\u{1F354}","\u{1F3CB}\uFE0F","\u{1F3A8}","\u{1F3B5}","\u{1F4AC}","\u2764\uFE0F"].map(emo => (
                        <button key={emo} onClick={e => { e.stopPropagation(); onEmojiChange(emo); }} style={{ fontSize:16, background: entry.emoji === emo ? `${C.accent}22` : "none", border: entry.emoji === emo ? `1px solid ${C.accent}55` : `1px solid ${C.border}`, borderRadius:6, width:32, height:32, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"all .1s" }}>{emo}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom:14 }}>
                    <div style={{ fontSize:10, fontWeight:700, color: C.dim, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:8, display:"flex", alignItems:"center", gap:8 }}>
                      &#128247; Photos
                      {(entry.images||[]).length > 0 && <span style={{ color: C.accent, fontWeight:400, textTransform:"none", letterSpacing:0 }}>&#183; {(entry.images||[]).length}</span>}
                      <button onClick={onAddPhoto} style={{ fontSize:10, color: C.accent, background:"none", border:`1px solid ${C.accent}44`, borderRadius:4, padding:"1px 7px", cursor:"pointer", fontFamily:"inherit", fontWeight:400, letterSpacing:0, textTransform:"none", marginLeft:"auto" }}>+ Add</button>
                    </div>
                    {(entry.images||[]).length === 0 && <div style={{ fontSize:11, color: C.dimmer, fontStyle:"italic", marginBottom:4 }}>No photos attached.</div>}
                    <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                      {(entry.images||[]).map((src: string, idx: number) => (
                        <div key={idx} style={{ position:"relative" }}>
                          <img src={src} alt="" style={{ width: isMobile ? 90 : 110, height: isMobile ? 90 : 110, objectFit:"cover", borderRadius:9, border:`1px solid ${C.border}`, display:"block" }} />
